...
export default function DownloadScreen() {
  ...
  const downloadUrl = useMemo(() => {
    ...
    const path =
      params.type === "tv" && params.season && params.episode
  ...
  // ── Enqueue download through the download engine ──
    ...
    async (url: string) => {
      try {
        ...
        const id = enqueue({
          url,
          fileName: filename,
          server: params.type === "tv" ? "nxsha" : "nxsha",
  ...
  const handleMessage = useCallback(
    (event: any) => {
      try {
        ...
        addLog(
          `Msg: ${msg.type}: ${typeof msg.data === "string" ? msg.data.substring(0, 200) : JSON.stringify(msg.data).substring(0, 200)}`,
        );
  ...
  return (
    <View className="flex-1 bg-black">
      ...
      <View
        style={{
          paddingBottom: Math.max(insets.bottom + 12, showDebug ? 220 : 12),
        }}
          ...
          {capturedUrls.length > 0 && (
            <View className="bg-zinc-900/95 rounded-xl border border-amber-500/30 mx-4 p-3 w-[90%]">
              <Text className="text-amber-400 text-xs font-bold mb-2">
                📥 {capturedUrls.length} Download URL
                {capturedUrls.length > 1 ? "s" : ""} Captured
              </Text>
              <ScrollView
                horizontal
              ...
              <View className="flex-row mt-2" style={{ gap: 6 }}>
                <TouchableOpacity
                  onPress={() => {
                    ...
                    addLog(
                      "Opening in system browser: " + url.substring(0, 200),
                    );
                    Linking.openURL(url).catch(() =>
                      showStatus("Cannot open URL"),
                    );
                  }}
            ...
            <ScrollView className="flex-1 px-2 py-1">
              {debugLogs.length === 0 ? (
                ...
                debugLogs.map((log, i) => (
                  <Text
                    key={i}
                    className="text-zinc-400 text-[9px] font-mono leading-tight mb-0.5"
                    selectable
                  >
                    {log}
                  </Text>
      ...
      <View className="flex-1">
        <WebView
          ...
          onError={(e) => {
            console.warn("[DL] Error:", e.nativeEvent.description);
            setLoading(false);
          }}
          onOpenWindow={(e) => {
            ...
            if (url && !capturedUrlsRef.current.includes(url)) {
              ...
              if (
                ...
                Alert.alert("Download URL Found", url, [
                  ...
                  {
                    text: "Show Full",
                    onPress: () => Alert.alert("Download URL", url),
                  },
                  { text: "Close", style: "cancel" },
                ]);
              }
