...
/**
 * Synthetic provider entry for the native direct player (HDHub links). It
 * rides the normal provider list/switching flow but renders the native
 * HevcPlayer instead of a WebView — no guard scripts, no staged security
 * waterfall, no Cloudflare handling (nothing to protect for direct files).
 */
const DIRECT_PROVIDER: ProviderDefinition = {
  id: "direct",
  name: "direct",
  displayName: "Direct · HDHub",
  note: "Native player · Fast · Seekable",
  order: -1,
  baseUrl: "",
  embed: {
    movie: () => "",
...
export function VideoWebView({
   ...
   * once, in-session episode changes must never fall back to it — that was
   * the "switched to ep 2 but it started at ep 1's 70%" bug.
   */
  const startAtPropUsedRef = useRef(false);
  ...
  const providers = useMemo(() => {
    // The direct (native) provider rides first — it needs no WebView and no
    // security waterfall, and is the fastest path when links are healthy.
    // The shared registry also carries a legacy placeholder provider with
  ...
  /** Fullscreen state of the embedded native player (direct provider). */
  const [directFullscreen, setDirectFullscreen] = useState(false);
  ...
  // Direct fullscreen must drive BOTH flags: directFullscreen expands the
  // player area, while isFullscreen runs the watch page's fullscreen logic
  // (orientation lock, overlay auto-hide, hiding non-player UI).
  const applyDirectFullscreen = useCallback(
    (fs: boolean) => {
      setDirectFullscreen(fs);
      setIsFullscreen(fs);
      if (fs) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // later episodes take their resume from their own progress only.
        startAtPropUsedRef.current = true;
  ...
  // episode) so "Next Episode" starts with zero network time.
  const [directNextEp, setDirectNextEp] = useState<{
    season: number;
    episode: number;
  } | null>(null);
  ...
  useEffect(() => {
    ...
    getNextEpisode(id, currentSeason, currentEpisode)
      .then(({ nextSeason, nextEpisode: nextEp, hasNext }) => {
        if (cancelled) return;
        if (
          !hasNext ||
          (nextSeason === currentSeason && nextEp === currentEpisode)
        ) {
          console.log(
            `[VideoWebView] Direct next episode: none (hasNext=${hasNext})`,
          );
          setDirectNextEp(null);
          return;
        }
        setDirectNextEp({ season: nextSeason, episode: nextEp });
        console.log(
  ...
  }, [isDirect, type, id, currentSeason, currentEpisode, settings]);
    ...
    <View className="flex-1 bg-black">
      ...
      {!(isDirect && directFullscreen) && (
        <PlayerControlOverlay
          ...
          isTV={isTV}
          loadState={isDirect ? { type: "PLAYING" } : loadState}
        ...
        <EpisodeRail
          ...
          onSelect={(s, e) => {
            ...
            if (s !== currentSeason || e !== currentEpisode) {
              if (isDirect) {
                // Direct links are per-episode — the host page re-fetches.
        ...
        {isDirect ? (
          ...
          directStream?.loading ? (
            ...
            <View className="flex-1 items-center justify-center px-8">
              ...
              <Text
                className="text-sm mb-6 text-center leading-5 max-w-xs"
                style={{ color: colors.textSecondary }}
              >
                {directStream.error || "No streams available"}
              </Text>
              <View
              ...
              >
                <TouchableOpacity
                  onPress={onDirectRetry}
                  className="rounded-xl py-3 px-5 flex-row items-center"
                  style={{ backgroundColor: colors.gold }}
                  activeOpacity={0.8}
                >
            ...
            <HevcPlayer
              ...
              prevalidatedResults={directStream.prevalidated}
              ...
              onNextEpisode={(s, e) => {
                // Same path as the episode picker: re-seed season/episode and
                // let the host page re-fetch per-episode direct links.
                setCurrentSeason(s);
                setCurrentEpisode(e);
                onDirectEpisodeChange?.(s, e);
              }}
                  ...
                  try {
                    ...
                    if (data.type === "anime-410") {
                      ...
                      const nextCombo = FALLBACK_QUEUE.find(
                        (c) =>
                          !triedCombos.current.has(`${c.space}:${c.audio}`),
                      );
