 ...
 */
function removePageSkipBtn(webViewRef: React.RefObject<PlayerWebViewRef | null>) {
  webViewRef.current?.injectJavaScript(`
(function(){
  var el = document.getElementById('__rn_skip_btn');
...
export function VideoWebView({
  ...
  const [introSegments, setIntroSegments] = useState<IntroDbResponse | null>(null);
  const introCheckTimerRef = useRef<ReturnType<typeof setInterval> | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      } catch (e) { console.warn('[WatchHistory] âŒ Resume check error:', e); }
    })();
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const segments = await fetchIntroSegments(imdbId, currentSeason, currentEpisode);
        if (!cancelled) {
          setIntroSegments(segments);
          if (segments?.intro) {
            console.log('[IntroDetect] Found intro: ' + segments.intro.start_sec + 's â€“ ' + segments.intro.end_sec + 's (conf=' + segments.intro.confidence + ')');
          }
          if (segments?.recap) {
            console.log('[IntroDetect] Found recap: ' + segments.recap.start_sec + 's â€“ ' + segments.recap.end_sec + 's');
          }
        }
      } catch (e: any) {
        console.warn('[IntroDetect] Fetch failed: ' + (e?.message ?? String(e)));
      }
    })();
  ...
  // Ã¢â€â‚¬Ã¢â€â‚¬ Periodic force-save (every 15s) Ã¢â€â‚¬Ã¢â€â‚¬
  useEffect(() => {
