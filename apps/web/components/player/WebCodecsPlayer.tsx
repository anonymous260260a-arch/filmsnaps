...
export function WebCodecsPlayer({
  ...
  const configureVideoDecoder = useCallback(
    async (track: TrackMeta) => {
      ...
      console.log(
        "[WebCodecs] Configuring VideoDecoder:",
        chosenCodec,
        track.width,
        track.height,
      );
  ...
  const configureAudioDecoder = useCallback(async (track: TrackMeta) => {
    if (!mountedRef.current) return;
    ...
    console.log(
      "[WebCodecs] Configuring AudioDecoder:",
      codec,
      track.sampleRate,
      track.channels,
    );
    ...
    try {
      ...
      if (!supported.supported) {
        console.warn(
          "[WebCodecs] AudioDecoder not supported, falling back to AudioContext only",
        );
        return;
      }
    } catch {
      console.warn(
        "[WebCodecs] AudioDecoder config check failed, using AudioContext only",
      );
      return;
    }
    ...
    try {
      ...
      console.warn(
        "[WebCodecs] AudioDecoder configure() threw — using AudioContext only:",
        e,
      );
      return;
    }
  ...
  const scheduleAudio = useCallback((audioData: AudioData) => {
    ...
    try {
      ...
      const offsetSec =
        (audioData.timestamp - firstAudioTsRef.current) / 1_000_000;
  ...
  const renderLoop = useCallback(() => {
    ...
    if (!isPaused && (videoDecoderRef.current?.decodeQueueSize ?? 0) > 0) {
      animFrameRef.current = requestAnimationFrame(renderLoop);
    ...
    (async () => {
      try {
        ...
        parser.onTrack = async (track: TrackMeta) => {
          if (!mountedRef.current) return;
          console.log(
            `[WebCodecs] Track #${track.trackNumber}: type=${track.trackType}, codec=${track.codecId}, lang=${track.language}`,
          );
        ...
        parser.onBlock = (block: ParsedBlock) => {
          ...
          const track = decoderInitTrack.find(
            (t) => t.trackNumber === block.trackNumber,
          );
          if (!track) return;
        ...
        const checkAudioTracks = setInterval(() => {
          if (decoderInitTrack.length > 0 && videoDecoderRef.current) {
            const audioTracksList = decoderInitTrack.filter(
              (t) => t.trackType === 2,
            );
    ...
    return (
      <div className="absolute inset-0 flex flex-col items-center justify-center bg-[#070708] z-40 gap-4 px-6">
        ...
        <p className="text-sm text-muted-foreground text-center max-w-xs">
          {errorMessage ||
            "Your browser does not support HEVC decoding via WebCodecs."}
        </p>
        <p className="text-xs text-faint text-center max-w-sm">
          Try selecting an H.264 (x264) quality option instead, or use Chrome on
          macOS/Android.
        </p>
      </div>
    );
