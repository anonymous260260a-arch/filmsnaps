/**
 * @deprecated This module is the R7 fallback in the R0-R8 rule cascade.
 *
 * The R0-R8 cascade (apps/desktop/src/security/rule-cascade.ts) provides:
 ...
 *   R4:  @cliqz/adblocker FiltersEngine — Aho-Corasick O(L) matching (100k+ rules)
 *   R5:  Always-block domains (blocklist.json) — block
 *   R6:  Always-block path patterns (blocklist.json) — block
 *   R7:  This module (legacy) — block
 *   R8:  Default allow
 *
 * This file is kept as-is for backward compatibility. All new blocking
 * patterns should go into blocklist.json's rules.alwaysBlock section.
 *
...
/**
 * @deprecated Use shouldBlockRequest() from rule-cascade.ts instead (R7 fallback).
 * The R0-R8 cascade at apps/desktop/src/security/rule-cascade.ts now provides
 * comprehensive blocking via @cliqz/adblocker (100k+ EasyList/AdGuard rules),
 * session trust, per-provider profiles, and cosmetic CSS.
 *
 * This function is kept as the R7 fallback for backward compatibility.
 *
...
export function getBlockCategory(url: string): string {
  ...
  for (const domain of DOMAIN_BLOCKLIST) {
    if (lower.includes(domain)) {
      ...
      if (domain.includes("analyt") || domain.includes("track"))
        return "tracker";
      if (domain.includes("miner") || domain.includes("coin"))
        return "crypto-miner";
      return "ad";
    }
  }
