...
- **Direct-stream mobile player** — when a provider exposes a direct link, the app probes and validates it on-device (binary container-signature check), streams it through expo-video with a vendored MKV secondary-SeekHead extractor (`patches/expo-video@55.0.18.patch`), per-format buffer profiles, automatic fallback across sources, and remember-the-working-server promotion; falls back to the sandboxed WebView player when no direct link survives.
