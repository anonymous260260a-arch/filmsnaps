...
describe("Provider Registry", () => {
  describe("PROVIDERS array", () => {
    ...
    it("every provider should have required fields", () => {
      for (const provider of PROVIDERS) {
        ...
        // Providers whose URL is built at runtime (e.g. "direct") have baseUrl: ""
        if (provider.baseUrl) {
    ...
    it("every provider baseUrl should be a valid URL", () => {
      for (const provider of PROVIDERS) {
        // Skip providers whose URL is built at runtime
  ...
  describe("getProvider", () => {
    ...
    it("should find all enabled providers by id", () => {
      const enabledIds = PROVIDERS.filter((p) => p.enabled !== false).map(
        (p) => p.id,
      );
  ...
  describe("getEnabledProviders", () => {
    ...
    it("should not include disabled providers", () => {
      const enabled = getEnabledProviders();
      const disabledIds = PROVIDERS.filter((p) => p.enabled === false).map(
        (p) => p.id,
      );
