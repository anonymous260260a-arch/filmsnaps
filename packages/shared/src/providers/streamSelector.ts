/**
 * Stream selection / ranking algorithm — THE canonical version.
 *
 * Ported from apps/mobile/lib/streamSelector.ts (the original canonical
 * ranking). It answers: "of these N links, which one starts fast, plays in
 * my language, and seeks without buffering?"
 *
 ...
 *
 * The chain's total order (see the comparator in rankStreams):
 *   1. demoted hosts (habitual stallers) last, whatever they carry.
 *   2. language bucket — the user's first priority (preferred / multi /
 *      hindi / english / unknown).
 ...
 * No speed test yet → assume 10 Mbps (6 Mbps after the 0.6 sustain factor).
 *
 * Quality is parsed from the FILENAME first — the API's `quality` field is
 * unreliable (it labels 1080p WEB-DL files as 2160p).
 *
 * This module is platform-agnostic: it knows nothing about NetInfo,
 * AsyncStorage or the browser. Callers resolve those via `selectBestStream`
 * (async convenience wrapper) or by injecting `isCellular` / `speedMbps`
 * directly into `rankStreams`.
 */
...
export const QUALITY_ORDER = ["8k", "4k", "1080p", "720p", "480p"] as const;
 ...
 */
export const SEEK_HEADROOM = 0.35;
/**
 ...
 * the cap" means "plays without buffering".
 */
export const CDN_SUSTAIN_FACTOR = 0.6;
/** Assumed speed when no speed test has run yet (Mbps). */
export const DEFAULT_SPEED_MBPS = 10;
export const DEFAULT_RUNTIME_MINUTES = 120;
export const TV_RUNTIME_MINUTES = 45;
...
export interface SelectOptions {
  ...
  /** Injected by the platform (NetInfo / navigator.connection). Default false. */
  isCellular?: boolean;
  /**
   * Injected by the platform — the connection speed in Mbps AFTER the
   * CDN_SUSTAIN_FACTOR calibration (see selectBestStream wrappers).
   * When absent, defaults to (isCellular ? 4 : 10) × CDN_SUSTAIN_FACTOR.
   */
  speedMbps?: number;
}
 ...
 */
export function sizeOf(link: StreamLink): number {
...
/**
 * Rank a pool of streams into ONE canonical priority chain.
 *
 * Pure and synchronous — network info is injected through options, so the
 * same code runs identically on web (navigator.connection) and mobile
 * (NetInfo + speed cache). See `selectBestStream` for the async wrappers.
 */
export function rankStreams(
  ...
  const speedMbps =
    injectedSpeedMbps ??
    (isCellular ? 4 : DEFAULT_SPEED_MBPS) * CDN_SUSTAIN_FACTOR;
...
/**
 * Async convenience wrapper — same result as `rankStreams`. Platform
 * wrappers (app-level lib/streamSelector modules) inject real network info.
 */
export async function selectBestStream(
  links: StreamLink[],
  options: SelectOptions = {},
): Promise<StreamSelection> {
  return rankStreams(links, options);
}

// re-export detectLanguage for wrapper use (kept unexported in older copies)
